...
if ( isNaN(dni) || dni < 0 ){
	...
	var lockup = ["T","R","W","A","G","M","Y","F","P","D","X","B","N","J","Z","S","Q","V","H","L","C","K","E"];
	var result = '';

for (var i = 0 ; i < lockup.length; i++){
...
if (array_words[i].length > largest) {
largest = array_words[i].length;
...
{
	var sum = 0;
	for (var j = 0, length = array_lengths.length; j < length; j++) {
	 	sum += array_lengths[j];
	 }; 
	return sum / length ;
}
